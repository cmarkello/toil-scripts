...
def run_indexing(job, options):
    ...
    #Split fastq files
    return job.addChildJobFn(run_split_fastq, options, index_dir_id, work_dir, cores=4, memory="4G", disk="2G").rv()
...
def run_split_fastq(job, options, index_dir_id, work_dir):
    ...
    num_chunks = 0
    for chunk_id, batch in enumerate(batch_iterator(record_iter, 31312)):
        num_chunks += 1
        chunk_id = chunk_id + 1
        ...
        job.addChildJobFn(run_alignment, options, filename_key, chunk_id, index_dir_id, work_dir, cores=4, memory="12G", disk="2G")
    
    return job.addFollowOnJobFn(run_merge_gam, options, num_chunks, index_dir_id, work_dir, cores=4, memory="4G", disk="2G").rv()
...
def run_merge_gam(job, options, num_chunks, index_dir_id, work_dir):
    ...
    if options.path_name and options.path_size:
        ...
        return_value = job.addFollowOnJobFn(run_merge_vcf, options, index_dir_id, work_dir, vcf_file_key_list, cores=4, memory="8G", disk="2G").rv()
    else:
...
def run_calling(job, options, index_dir_id, alignment_file_key, work_dir, path_name, path_size):
    ...
    # run chunked_call
    chunks = dockered_chunked_call(job, options, out_store, work_dir, index_dir_id, job.cores, xg_file, alignment_file, path_name, path_size, options.sample_name, variant_call_dir)

    vcf_file_key = job.addFollowOnJobFn(merge_vcf_chunks, options, work_dir, index_dir_id, path_name, path_size, chunks, options.overwrite, cores=4, memory="8G", disk="2G").rv()
...
def dockered_chunked_call(job, options, out_store, work_dir, index_dir_id, threads, xg_path, gam_path, path_name, path_size, sample_name, out_dir, chunk=10000000,
    ...
    for chunk_i, chunk in enumerate(chunks):
        ...
        job.addChildJobFn(call_chunk, options, work_dir, index_dir_id, xg_path, path_name,
                           ...
                           sample_name, threads,
                           overwrite, cores=4, memory="12G", disk="2G")
    return chunks
...
def run_upload(job, options, uploadList):
    ...
    return job.addChildJobFn(run_indexing, options, cores=4, memory="12G", disk="2G").rv()
